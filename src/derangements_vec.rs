...
where
    ...
    usize: TryFrom<T>,
{
    derangements_old(iterable.into_iter(), k)
}
...
/// ```
/// use itertools::{assert_equal, Itertools};
/// use derangements::derangements_old;
/// assert_equal(derangements_old(vec![0, 1, 2].into_iter(), 3), [[1, 2, 0], [2, 0, 1]]);
///
/// // There can be repeated values in the input, which will not be deduplicated
/// assert_equal(derangements_old(vec![0, 1, 1].into_iter(), 3), [[1, 0, 1], [1, 0, 1]]);
///
/// // The length of the derangements can be shorter than the input iterable
/// assert_equal(derangements_old(vec![0, 1, 2].into_iter(), 2), [[1, 0], [1, 2], [2, 0]]);
///
/// // There can be values that are outside the range of the indices
/// assert_equal(derangements_old(vec![0, 1, -1].into_iter(), 3), [[1, 0, -1], [1, -1, 0], [-1, 0, 1]]);
/// ```
pub fn derangements_old<T, U>(iterable: T, k: usize) -> Vec<Vec<U>>
where
    T: Iterator<Item = U>,
    U: Clone,
    usize: TryFrom<U>,
...
/// ```
/// use itertools::{assert_equal, Itertools};
/// use derangements::{derangements_old, derangements_vec, distinct_derangements_vec};
///
